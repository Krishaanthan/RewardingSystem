              ...
              <div>
                ...
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-left text-white/90 outline-none focus:border-white/50 focus:ring-1 focus:ring-white/50 transition-colors hover:bg-white/15"
                    aria-haspopup="listbox"
                    aria-expanded={isDropdownOpen}
                  >
                    <span className={`truncate ${!selectedActivity ? "text-white/50" : "text-white/90"}`}>
                      {selectedActivity ? `${selectedActivity} (${ACTIVITY_REWARDS[selectedActivity]} pts)` : "Select activity"}
                    </span>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`h-4 w-4 text-white/50 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                    </svg>
                  </button>

                  <div className={`absolute left-0 mt-2 w-full z-50 origin-top rounded-xl border border-white/20 bg-[#6b0a23]/95 p-1 backdrop-blur-xl shadow-2xl transition-all duration-200 ease-in-out ${isDropdownOpen ? "scale-100 opacity-100" : "pointer-events-none scale-95 opacity-0"}`}>
                    <ul className="max-h-60 overflow-y-auto rounded-lg text-sm text-white/90 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/20">
                      {activities.map((activity) => (
                        <li
                          ...
                          className={`cursor-pointer rounded-md px-4 py-3 transition-colors hover:bg-white/15 ${selectedActivity === activity ? 'bg-white/20 font-semibold text-white' : ''}`}
                        >
                      ...
                      field.type === "link" ? (
                        <div key={field.id}>
                          ...
                          <input
                            ...
                            className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white/90 outline-none placeholder:text-white/40 focus:border-white/50 focus:ring-1 focus:ring-white/50"
                          />
                        </div>
                      ) : (
                        <div key={field.id}>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (span) {
                                  ...
                                  span.classList.replace("text-white/50", "text-white");
                                }
                              }}
                            />
                            <span className="flex-1 truncate px-4 py-3 text-sm text-white/50">
                              No file chosen
                            </span>
                            <span className="m-1 flex-shrink-0 cursor-pointer rounded-lg bg-[#8F113B] px-5 py-2 text-sm font-semibold text-white transition hover:bg-[#a61a49] peer-valid:bg-brand-primary">
                              Choose file
                            </span>
                          </div>
                        </div>
