...
export default function SubmissionStatusesPage() {
  return (
    <div className="relative min-h-screen w-full overflow-hidden text-white font-primary bg-black">
      ...
      <div className="fixed inset-0 z-0 bg-black/30 bg-gradient-to-b from-black/40 via-transparent to-black/40 mix-blend-multiply" />

      {/* Main Content Container (relative z-10 for scrollability over fixed bg) */}
      <div className="relative z-10 mx-auto flex min-h-screen max-w-5xl flex-col px-6 py-6 overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/20">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/20 pb-4">
          ...
          <nav className="mt-4 flex gap-4 md:mt-0 text-sm font-medium">
            ...
            <Link
              ...
              className="rounded-full border border-white/30 bg-white/10 px-6 py-2 backdrop-blur-md transition hover:bg-white/20 text-white"
            >
              Submission Statuses
            </Link>
          </nav>
        ...
        {/* Footer */}
        <footer className="mt-12 pb-8 text-center text-xs text-white/50">
